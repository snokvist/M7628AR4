#!/bin/sh
# set_rate.sh <if> <mcs> [bw_mhz] [sgi] [wcid] [stbc] [ldpc]
#   <if>      : ra0 | apcli0 | ...
#   <mcs>     : HT 0..32
...
#     Mode=2 (HT), BW:0=20/1=40, VhtNss=0 (HT), SGI:0/1, Preamble:1

set -e

err(){ echo "[set_rate] ERROR: $*" >&2; exit 1; }
usage(){ echo "Usage: $0 <if> <mcs 0..32> [20|40] [sgi 0|1] [wcid] [stbc 0|1] [ldpc 0|1]" >&2; exit 1; }

IF="$1"; MCS="$2"; BWmhz="${3:-20}"; SGI="${4:-0}"; WCID="${5:-1}"; STBC="${6:-0}"; LDPC="${7:-0}"
[ -n "$IF" ] && [ -n "$MCS" ] || usage
...
case "$STBC" in 0|1) ;; *) err "stbc must be 0 or 1";; esac
case "$LDPC" in 0|1) ;; *) err "ldpc must be 0 or 1";; esac
...
echo "[set_rate] iface=$IF mode=ht mcs=$MCS bw=${BWmhz}MHz sgi=$SGI stbc=$STBC ldpc=$LDPC wcid=$WCID"
